...
class IEEECaseLoader:
    ...
    def get_system_matrices(self) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        """
        n = self.get_state().n_gen

        # Simplified swing equation matrices
        # State: x = [delta, omega] where delta is rotor angle, omega is frequency
        # dx/dt = A*x + B*u

        # Typical values (can be refined with actual machine parameters)
        M = torch.ones(n) * 5.0   # Inertia constant (s)
        D = torch.ones(n) * 2.0   # Damping coefficient

        # A matrix: block structure
        # [0, I; -K/M, -D/M]
        # For simplified analysis, use diagonal approximation
